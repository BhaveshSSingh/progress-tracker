  ...
  return (
    <style dangerouslySetInnerHTML={{
      __html: Object.entries(config)
        .map(([key, itemConfig]) => {
          const color =
            itemConfig.theme?.dark || itemConfig.color || itemConfig.theme?.light
          return `
            [data-chart=${id}] {
              --color-${key}: ${color};
            }
...
>(({ active, payload, className, indicator = "dot", hideLabel = false, hideIndicator = false, label, labelFormatter, config, nameKey, labelKey }, ref) => {
  ...
  return (
    ...
    >
      {!nestLabel ? (
        <div className="mb-1 border-b border-border/50 pb-1.5 font-medium text-foreground">
          {label}
        </div>
      ) : null}
